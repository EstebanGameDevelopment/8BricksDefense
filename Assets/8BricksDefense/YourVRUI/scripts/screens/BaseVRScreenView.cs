				...
				if (m_refocus)
				{
					if (m_timeToRefocus > DELAY_TO_REFOCUS)
					{
						...
						if (!UtilitiesYourVRUI.IsVisibleFrom(canvasBounds, YourVRUIScreenController.Instance.GameCamera))
						{
							...
							iTween.MoveTo(this.gameObject, iTween.Hash("position", targetPosition, "easetype", iTween.EaseType.linear, "time", 0.95 * DELAY_TO_REFOCUS));
							m_timeToRefocus = 0;
						}
					}
				}
